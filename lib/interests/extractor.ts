...
// ── 오매칭 방지 블랙리스트 ────────────────────────────────────────────────
/**
 * 동사·광역 일반어로 인한 오매칭 차단:
 *
 *   - 동사·일반어: "배우"(배우다), "교사"(교사가 되고 싶다 → 14개 학과)
 *   - 광역 학문어: "철학", "문학", "통계" 등 → 어느 학과나 걸림
 *   - 광역 동사: "연구", "개발", "분석", "관리" → 직무어이지 학과 신호 아님
 *   - 영어 약어: "AI" → 너무 짧고 범용
 *
 * 이 단어들은 추출 단계에서 모두 무시됩니다. (학과 매핑 자체는 의미 보존)
 */
const KEYWORD_BLACKLIST = new Set([
  ...
  // 너무 많은 학과에 매칭되는 직업어
  "교사", "임용고시",
  // 광역 학문·분야명 (학과별 변별력 없음)
  "교육", "정책", "문학", "통계", "외교", "임상",
  "철학", "연구", "과학", "기술", "경영", "관리", "개발",
  "설계", "분석", "기획", "AI", "공학", "역사", "문화",
  "사회", "환경", "경제", "법학", "의학",
]);
